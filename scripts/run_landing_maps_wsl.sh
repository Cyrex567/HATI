...
for test in channels footprint coreg kinematics audit_regressions shadow_likelihood landing_maps shadow_diagnostics scene_diagnostics; do
...
for sigma in 0.25 0.5 1.0; do
  python scripts/landing_maps.py \
    ...
    --output "output/athena/landing_maps_v255/$RUN/registration-$sigma" \
    2>&1 | tee "logs/landing-maps-registration-$sigma-$RUN.log"
done
python scripts/compare_landing_scenarios.py \
  "output/athena/landing_maps_v255/$RUN/registration-0.25" \
  "output/athena/landing_maps_v255/$RUN/registration-0.5" \
  "output/athena/landing_maps_v255/$RUN/registration-1.0" \
  --output "output/athena/landing_maps_v255/$RUN/comparison" \
  2>&1 | tee "logs/landing-maps-comparison-$RUN.log"
echo "Done. Three independent map files + fusion observability in each scenario:"
echo "output/athena/landing_maps_v255/$RUN"
