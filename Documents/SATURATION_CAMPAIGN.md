...
The wrapper starts HATI Watch on `http://localhost:8765` and opens it in the Windows browser. It then runs the software checks, the map replay, the earlier tests T1–T3, T5–T7 and T9–T11, and the relief and noise tests T12, T13, T14 and T16 with `configs/saturation_campaign_relief_workstation.json` into a new folder `output/athena/saturation_campaign/relief-<UTC time>`. T4 and T8 need external thermal and annotated data and are left out; `STAGES=T1,T12,T13,T14,T16` runs only the relief tests. The earlier tests keep their original control generator, so they reproduce the athena-watch-01 numbers as a regression check. The full list takes several hours on the workstation, most of it T2, T3 and the adaptive stages. The results ZIP is copied to `C:\Users\Public\Downloads\HATI`. After the run the viewer stays open until Enter is pressed. A first argument selects another bundle. `CONFIG`, `STAGES`, `OUT`, `EXPORT_DIR`, `PORT` and `NO_BROWSER=1` override the defaults. Exit codes are those of the runner. The first noise-only campaign (`noise-01`) used `configs/saturation_campaign_noise_workstation.json` with `STAGES=T1,T12,T16`.
